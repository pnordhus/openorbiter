	...
	m_gravityFactor(1.0f)
{
	...
	m_nodeTimer = g_config->getFloat("firstNodeTime");
...
void Game::process(float time)
{
	...
	if (m_nodes.size() > 1) {
		...
		if (m_nodeTimer <= 0.0f) {
			m_nodeTimer = g_config->getFloat("nextNodeTime");
			foreach (Player* player, m_players) {
	...
	for (unsigned int it = 0; it < iterations; it++) {
		Vector v = m_map->gravity() * g_config->getFloat("gravityFactor") * m_gravityFactor * time;
		foreach (Player* player, m_players) {
